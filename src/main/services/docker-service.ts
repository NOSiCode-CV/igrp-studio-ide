...
export class DockerService {
  ...
  async checkDockerDaemon(): Promise<{
    ...
    try {
      ...
      const errorMessage = (error as any).stderr || (error as any).stdout || (error as any).message
  ...
  async loadComposeFile(projectPath: string): Promise<DockerComposeConfig> {
    ...
    try {
      ...
      const serviceCount = Object.keys(this.composeCache[projectPath].services || {}).length
  ...
  private prepareInitScript(projectPath: string): void {
    ...
    try {
      ...
      for (let i = 0; i < shFiles.length; i++) {
        ...
        this.logProgress(i + 1, shFiles.length, `Preparing script: ${path.basename(scriptPath)}`)
  ...
  async executeComposeCommand(
    ...
    const fullCommand = `docker compose -f ${escapedComposeFile} ${command} ${serviceParam}`.trim()
  ...
  async up(projectPath: string): Promise<void> {
    ...
    try {
      ...
      await this.simulateProgress(
        () => this.executeComposeCommand(projectPath, `up -d --quiet-pull`),
  ...
  async down(projectPath: string, dropVolume: boolean): Promise<void> {
    this.logInfo(`Stopping Docker Compose services${dropVolume ? ' and removing volumes' : ''}...`)
    try {
    ...
    try {
      ...
      try {
        ...
        if (!dockerCheck.isRunning) {
          this.logWarn('Docker daemon not running, returning error status for all services')
        ...
        this.logDebug(`Found ${runningContainers.length} running containers`)
        ...
        const serviceInfo = serviceNames.map((serviceName) => {
          ...
          if (containerInfo) {
            ...
            return {
              ...
              ports:
                containerInfo.Publishers?.map((p: any) => `${p.PublishedPort}:${p.TargetPort}`) ||
                [],
              volumes: processedVolumes,
              environments: this.parseEnvironmentToArray(environment),
        ...
        this.logSuccess(`Status check completed for ${serviceNames.length} services`)
        return serviceInfo
      } catch (parseError) {
  ...
  async stop(projectPath: string, services: string[]): Promise<void> {
    ...
    try {
      await this.executeComposeCommand(projectPath, `stop ${services.join(' ')}`)
  ...
  async restart(projectPath: string, services: string[], timeout?: number): Promise<void> {
    ...
    try {
      ...
      await this.executeComposeCommand(projectPath, `restart ${timeoutFlag} ${services.join(' ')}`)

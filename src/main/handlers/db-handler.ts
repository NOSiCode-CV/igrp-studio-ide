...
ipcMain.handle(
    EVENTS.CONNECTION.GET_CONNECTIONS,
    async (_event): Promise<Array<Connection>> => {
...
ipcMain.handle(
    ...
    async (_event, connectionName: string) => {
    ...
    async (_event, connectionName): Promise<DatabaseResponse> => {
        try {
            const connectionConfig =
                await repoConnection.findOne(connectionName);
            if (!connectionConfig) {
                return { success: false, message: 'Connection not found' };
            }
            console.log('connectionConfig', connectionConfig);
            const knex =
                globalKnex || (await createKnexConnection(connectionConfig));
...
ipcMain.handle(
    ...
    async (_event, connectionName, tableName) => {
        try {
            const connectionConfig =
                await repoConnection.findOne(connectionName);
            if (!connectionConfig) {

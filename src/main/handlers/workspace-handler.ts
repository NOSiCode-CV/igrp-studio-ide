        ...
        event.sender.send(EVENTS.LOG, {
            ...
            message:
                error instanceof Error
                    ? error.message
                    : 'Failed to get last access failed:',
        });
        throw error;
    }
});
...
ipcMain.handle(
    EVENTS.REPOSITORY.WORKSPACE.DELETE,
...
ipcMain.handle(
    EVENTS.REPOSITORY.WORKSPACE.FIND_RECENT,
    async (_, limit = 5) => {
        try {
...
handleWithCustomErrors(
    ...
    async (
        _event,
        workspaceId: string,
        project: Omit<ProjectData, 'id' | 'createdAt'>
    ) => {
...
ipcMain.handle(
    ...
    async (_, projectId: string, basePath: string) => {
...
ipcMain.handle(
    EVENTS.REPOSITORY.PROJECT.FIND_ALL,
...
ipcMain.handle(
    ...
    async (_, serviceId: string, basePath: string) => {
        await repo.deleteService(serviceId, basePath);
    }
);

ipcMain.handle(
    EVENTS.REPOSITORY.SERVICE.FIND_ALL,
    async (_, workspaceId: string) => {
...
// Backup Handlers
ipcMain.handle(
    EVENTS.REPOSITORY.BACKUP.CREATE,
    async (event, backupPath?: string) => {
        try {
...
ipcMain.handle(
    EVENTS.REPOSITORY.BACKUP.RESTORE,
    async (event, backupPath: string) => {
        try {

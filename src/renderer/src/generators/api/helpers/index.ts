...
export const handleChangeValueObject = (
  ...
  const isObject =
    typeof result === 'object' && element !== 'relation' && !Array.isArray(result) && result.value
    ...
    newChildren.forEach((child: any) => {
      ...
      if (childrenMap[name]) {
        ...
        if (child.children && childrenMap[name].children) {
          childrenMap[name].children = mergeChildren(childrenMap[name].children!, child.children)
        }
      } else {

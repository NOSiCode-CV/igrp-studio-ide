...
export function getOptionsByObject(
    objects: any,
    module: string,
    ...
    return objects !== undefined
        ? objects
              .filter((m: any) => {
                  return !(
                      m.content?.module === module &&
                      m.content?.name === currentItem
                  );
              })
              .map((item: any) => ({
...
export const removeRow = (
    ...
    formik.setFieldValue(
        ...
        formik.values[field].filter(
            (_: any, index: number) => index !== position
        )
    );
};
...
export const extractByType = (moduleData: any, type: string) => {
    ...
    const typeFiles = files.find(
        (item: any) => item.name === type.toLowerCase()
    );
...
const mergeFilesByType = (files: FileTree[]): FileTree[] => {
    ...
    const mergeChildren = (
        existingChildren: FileTree[],
        newChildren: FileTree[]
    ): FileTree[] => {
...
export const getMergedFiles = (studio: any, module: string) => {
    const currentModuleData =
        studio.filesThree.find((item: any) => item.name === module) || {};
    const sharedModuleData =
        studio.filesThree.find((item: any) => item.name === 'shared') || {};

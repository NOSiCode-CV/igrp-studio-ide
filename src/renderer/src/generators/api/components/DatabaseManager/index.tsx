...
const DatabaseManagerModal: React.FC<DatabaseManagerModalProps> = ({
    ...
    const processTable = async (tableName: string): Promise<void> => {
      ...
      try {
        const { success, message, structure } =
          await window.igrpStudio.connection.getTableStructure(selectedConnection, tableName)
        ...
        if (success) {
          ...
          const attributes = structure
            ...
            .map((column: any) => {
              const relation = column.foreign_key_table
                ? {
                    type: 'ManyToOne',
                    entity: toFullCamelCaseFromSnakeCase(column.foreign_key_table),
          ...
          console.error(`Failed to fetch structure for table: ${tableName}. Message: ${message}`)
        }
      } catch (error) {
  ...
  return (
    <IGRPDialogPrimitive open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <IGRPDialogHeaderPrimitive>
          <IGRPDialogTitlePrimitive>{t('importDataTableFromDatabase')}</IGRPDialogTitlePrimitive>
          <IGRPDialogDescriptionPrimitive>
        ...
        <IGRPDialogFooterPrimitive>
          <IGRPDialogClosePrimitive asChild>
            <IGRPButtonPrimitive type="button" variant="secondary" onClick={() => handleClose}>
              {t('close')}
            </IGRPButtonPrimitive>
          </IGRPDialogClosePrimitive>

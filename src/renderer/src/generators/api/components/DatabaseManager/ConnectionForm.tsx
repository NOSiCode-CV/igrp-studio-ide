...
export function ConnectionForm({
  ...
  const handleTestConnection = async (
    ...
    e.preventDefault()
    const { success, message } = await window.igrpStudio.connection.connectToDatabase(values)
    if (success && message) {
  ...
  const validationSchema = Yup.object({
    ...
    port: Yup.number().required(t('PortRequired')).min(1, t('PortGreaterThanZero')),
        ...
        <Form className="space-y-4">
          ...
          >
            <IGRPTabsListPrimitive className="grid w-full grid-cols-2">
              ...
              <IGRPTabsTriggerPrimitive value="ssh">{t('sshConnection')}</IGRPTabsTriggerPrimitive>
            </IGRPTabsListPrimitive>
            <IGRPTabsContentPrimitive value="general">
              <div className="space-y-4 grid md:grid-cols-1">
                <div className="space-y-2">
                  <IGRPLabelPrimitive htmlFor="name">{t('connectionName')}</IGRPLabelPrimitive>
                  <IGRPInputPrimitive
                    id="name"
                    name="name"
                    value={values.name}
                ...
                <div className="flex flex-col space-y-2">
                  ...
                  <IGRPCombobox
                    ...
                    onChange={(selected: string | string[]) => {
                      setFieldValue('databaseType', selected as string)
                    }}
                    className="w-full"
                  />
                  {errors.databaseType && touched.databaseType && (
                    <div className="text-xs text-destructive">{errors.databaseType}</div>
                  )}
                </div>
                  ...
                  <div className="col-span-9">
                    <div className="space-y-2">
                      ...
                      {errors.host && touched.host && (
                        <div className="text-xs text-destructive">{errors.host}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="col-span-3">
                    <div className="space-y-2">
                      ...
                      {errors.port && touched.port && (
                        <div className="text-xs text-destructive">{errors.port}</div>
                      )}
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-2">
                    ...
                    {errors.user && touched.user && (
                      <div className="text-xs text-destructive">{errors.user}</div>
                    )}
                  </div>

                  <div className="space-y-2">
                    <IGRPLabelPrimitive htmlFor="password">{t('password')}</IGRPLabelPrimitive>
                    <IGRPInputPrimitive
                      id="password"
                      name="password"
                      type="password"
                ...
                <div className="space-y-2">
                  <IGRPLabelPrimitive htmlFor="database">{t('databaseName')}</IGRPLabelPrimitive>
                  <IGRPInputPrimitive
                    id="database"
                    name="database"
                    type="database"
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-12 gap-2">
                    <div className="col-span-9">
                      <div className="space-y-2">
                        <IGRPLabelPrimitive htmlFor="host">{t('databaseHost')}</IGRPLabelPrimitive>
                        <IGRPInputPrimitive
                          id="host"
                          name="host"
                          value={values.host}
                    ...
                    <div className="col-span-3">
                      <div className="space-y-2">
                        <IGRPLabelPrimitive htmlFor="port">{t('databasePort')}</IGRPLabelPrimitive>
                        <IGRPInputPrimitive
                          id="port"
                          name="port"
                          type="number"
                ...
                <div className="space-y-4 pt-4 border-t">
                  ...
                  <div className="grid grid-cols-12 gap-2">
                    <div className="col-span-9">
                      <div className="space-y-2">
                        <IGRPLabelPrimitive htmlFor="sshHost">{t('sshHost')}</IGRPLabelPrimitive>
                        <IGRPInputPrimitive
                          id="sshHost"
                          name="sshHost"
                        ...
                        {errors.sshHost && touched.sshHost && (
                          <div className="text-xs text-destructive">{errors.sshHost}</div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="col-span-3">
                      <div className="space-y-2">
                        <IGRPLabelPrimitive htmlFor="sshPort">{t('sshPort')}</IGRPLabelPrimitive>
                        <IGRPInputPrimitive
                          id="sshPort"
                          name="sshPort"
                          type="number"
                        ...
                        {errors.sshPort && touched.sshPort && (
                          <div className="text-xs text-destructive">{errors.sshPort}</div>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-2">
                      ...
                      {errors.sshUsername && touched.sshUsername && (
                        <div className="text-xs text-destructive">{errors.sshUsername}</div>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <IGRPInputPrimitive
                        ...
                        required
                      />
                      {errors.sshPassword && touched.sshPassword && (
                        <div className="text-xs text-destructive">{errors.sshPassword}</div>
                      )}
                    </div>
                  </div>
                </div>

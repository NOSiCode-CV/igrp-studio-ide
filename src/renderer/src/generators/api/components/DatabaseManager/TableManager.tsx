...
export function TableManager({
  ...
  const [connections, setConnections] = useState<{ label: string; value: string }[]>([])
  ...
  const handleTableSelect = async (table: string) => {
    ...
    const { success, message, structure } = await window.igrpStudio.connection.getTableStructure(
      selectedConnection,
      table
    )
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="space-y-2">
          <IGRPLabelPrimitive className="text-xs">{t('whileMatchingSchema')}</IGRPLabelPrimitive>
          <IGRPCombobox
            value={action}
        ...
        <div className="space-y-4">
          <div className="rounded border">
            ...
            <IGRPScrollAreaPrimitive className="h-[450px] w-full px-2">
              {isLoading ? (
                <div className="text-center">{t('loadingTables')}</div>
              ) : (
                <IGRPDataTable columns={columns} data={tables} />
              )}
            </IGRPScrollAreaPrimitive>
          </div>
        </div>
                ...
                <IGRPTablePrimitive>
                  <IGRPTableHeaderPrimitive>
                    <IGRPTableRowPrimitive>
                      ...
                      <IGRPTableHeadPrimitive>{t('dataType')}</IGRPTableHeadPrimitive>
                      <IGRPTableHeadPrimitive>{t('isNullable')}</IGRPTableHeadPrimitive>
                    </IGRPTableRowPrimitive>
                  </IGRPTableHeaderPrimitive>
                  <IGRPTableBodyPrimitive>
                    {previewColumns &&
                      previewColumns.map((column, key) => (
                        <IGRPTableRowPrimitive key={key}>
                          <IGRPTableCellPrimitive>{column?.name}</IGRPTableCellPrimitive>
                          <IGRPTableCellPrimitive>{column?.data_type}</IGRPTableCellPrimitive>
                          <IGRPTableCellPrimitive>
                            {column?.is_nullable ? t('yes') : t('no')}
                          </IGRPTableCellPrimitive>
                        </IGRPTableRowPrimitive>
                      ))}
                  </IGRPTableBodyPrimitive>

...
export function JSONSchemaBuilder({
    ...
    if (initialSchema) {
      return {
        ...
        properties: Object.fromEntries(
          Object.entries(initialSchema.properties || {}).map(([key, value]) => [key, { ...value }])
        )
      }
    }
  ...
  const [fieldOrder, setFieldOrder] = useState<string[]>(() =>
    initialSchema && initialSchema?.properties ? Object.keys(initialSchema.properties) : []
  )
        ...
        const updateProperties = (
          ...
          return Object.fromEntries(
            Object.entries(properties).map(([id, field]) => {
              if (id === parentId) {
                if (field.type === 'object' || field.type === 'array') {
                  ...
                  const updatedField = {
                    ...field,
                    properties: Object.fromEntries(
                      Object.entries(updatedProperties).map(([_key, value]) => {
                        const updatedValue = checkAndUpdateDuplicateNames(
                          value
                          /*  field,
                                                        existingNames */
              ...
              } else if ((field.type === 'object' || field.type === 'array') && field.properties) {
                return [
                  id,
                  {
                    ...field,
                    properties: updateProperties(field.properties)
                  }
                ]
              }
  ...
  const handleUpdateField = useCallback(
    (id: string, updatedField: SchemaField, isNewField: boolean = false) => {
      const updateProperties = (
        properties: Record<string, SchemaField>
      ): Record<string, SchemaField> => {
        if (id in properties) {
        ...
        return Object.fromEntries(
          Object.entries(properties).map(([key, field]) => {
            if ((field.type === 'object' || field.type === 'array') && field.properties) {
              return [
                key,
                {
                  ...field,
      ...
      if (isNewField) {
        ...
        setFieldOrder((prev) => prev.map((field) => (field === id ? updatedField.name : field)))
      }
    },
    []
  )
  const handleDeleteField = useCallback((id: string, isNewField: boolean = false) => {
    if (isNewField) {
      setNewFields((prev) => {
        const { [id]: _, ...rest } = prev
        return rest
            ...
            Object.entries(properties).map(([key, field]) => {
              if ((field.type === 'object' || field.type === 'array') && field.properties) {
                return [
                  ...
                  {
                    ...field,
                    properties: deleteFromProperties(field.properties)
                  }
                ]
              }
              return [key, field]
            ...
            <IGRPTableHeadPrimitive className="text-right flex flex-1 items-center">
              {fieldOrder.length === 0 && Object.entries(newFields).length === 0 && (
                <IGRPTooltipPrimitive>
                  ...
                  <IGRPTooltipContentPrimitive>{t('addNewField')}</IGRPTooltipContentPrimitive>
                </IGRPTooltipPrimitive>
              )}
        ...
        <IGRPTableBodyPrimitive>
          {fieldOrder.map((id) => (
            <SchemaFieldRow
              ...
              depth={0}
              onUpdate={(updatedField, _index) => handleUpdateField(id, updatedField, false)}
          ...
          {Object.entries(newFields).map(([id, field]) => (
            <SchemaFieldRow
              ...
              field={field}
              depth={0}
              onUpdate={(updatedField) => handleUpdateField(id, updatedField, true)}

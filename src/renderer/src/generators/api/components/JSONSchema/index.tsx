...
import type { JSONSchema, SchemaField } from '../../types/schema'
        ...
        if (initialSchema) {
            return {
                ...
                properties: Object.fromEntries(
                    Object.entries(initialSchema.properties || {}).map(([key, value]) => [
                        key,
                        { ...value }
                    ])
                )
                            ...
                            if (id === parentId) {
                                if (field.type === 'object' || field.type === 'array') {
                                    ...
                                    const updatedField = {
                                        ...field,
                                        properties: Object.fromEntries(
                                            Object.entries(updatedProperties).map(
                                                ([_key, value]) => {
                                                    const updatedValue =
    ...
    const handleUpdateField = useCallback(
        (id: string, updatedField: SchemaField, isNewField: boolean = false) => {
                    ...
                    return {
                        ...
                        [updatedField.name]: {
                            ...
                            properties: updatedField.properties
                                ? Object.fromEntries(
                                      Object.entries(updatedField.properties).map(
                                          ([_key, field]) => [
                                              field.name,
                                              {
                                                  ...field
            ...
            if (isNewField) {
                ...
                setFieldOrder((prev) =>
                    prev.map((field) => (field === id ? updatedField.name : field))
                )
            }
        },
        []

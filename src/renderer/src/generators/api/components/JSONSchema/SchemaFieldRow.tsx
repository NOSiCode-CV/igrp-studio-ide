...
export function SchemaFieldRow({
  ...
  const [isExpanded, setIsExpanded] = useState(field?.type === 'object' || field?.type === 'array')
  ...
  const checkForDuplicateName = (newName: string): boolean => {
    if ((type === 'object' || type === 'array') && field?.properties) {
      return Object.keys(field.properties).some((key) => key !== field.name && key === newName)
    }
    return false
  }
      ...
      <IGRPTableRowPrimitive className={`group group/opt ${isNew ? 'bg-muted/50' : ''}`}>
        ...
        >
          ...
          <IGRPInputPrimitive
            ...
            placeholder={
              index !== undefined && type === 'array' ? `Item ${index + 1}` : 'fieldName'
            }
          />
        </IGRPTableCellPrimitive>

...
export function CreateEndpointDialog({
  ...
  const validationSchema = Yup.object({
    ...
    module: Yup.string().required(t('thisFieldRequired', { name: 'Module' }))
  })
        ...
        <form onSubmit={formik.handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-12 space-y-3">
                <Label htmlFor="basePath" className="text-right">
                  Base path
                </Label>
                <Input
                  id="basePath"
                  ...
                  className={cn(
                    formik.touched.basePath && formik.errors.basePath ? 'border-red-500' : ''
                  )}
                />
                {formik.errors.basePath && (
                  <p className="text-sm text-red-600">{formik.errors.basePath}</p>
                )}
              </div>
              <div className="col-span-12 space-y-3">
                ...
                <Combobox
                  ...
                  onChange={(value) => formik.setFieldValue('module', value)}
                  className={cn(
                    'w-full',
                    formik.touched.module && formik.errors.module ? 'border-red-500' : ''
                  )}
                />
                {formik.errors.module && (
                  <p className="text-sm text-red-600">{formik.errors.module}</p>
                )}
              </div>
            </div>
          </div>

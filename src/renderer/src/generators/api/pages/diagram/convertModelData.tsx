...
export function convertModelData(inputModels: Array<any>): {
    ...
    const convertAttribute = (attr: any): Attribute => ({
        ...
        color: attr.primaryKey ? 'purple' : attr.type === 'string' ? 'green' : 'orange'
    })

    const models: ModelData[] = inputModels.map((inputModel) => {
        const { content: model } = inputModel
        const regularAttributes = model.attributes.filter((attr: any) => attr.type !== 'relation')
        const inheritedAttributes = model.attributes.filter((attr: any) => attr.type === 'relation')
        return {
            key: model.name,
            name: model.name,

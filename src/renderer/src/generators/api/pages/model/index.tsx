            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-2 gap-4 mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <IGRPLabelPrimitive htmlFor="audit">{t('auditModel')}</IGRPLabelPrimitive>
                </div>
                ...
                {enableEntityRevision && (
                  <div className="flex items-center space-x-2">
                    <IGRPCheckboxPrimitive
                      id="revision"
                      onCheckedChange={(checked) => formik.setFieldValue('revision', checked)}
                      checked={formik.values.revision}
                    />
                    <IGRPLabelPrimitive htmlFor="revision">{t('revision')}</IGRPLabelPrimitive>
                  </div>
                )}
                <div className="flex items-center space-x-2">
                  <IGRPCheckboxPrimitive
                    id="crud"
                    onCheckedChange={(checked) => formik.setFieldValue('crud', checked)}
                    checked={formik.values.crud}
                  />
                  <IGRPLabelPrimitive htmlFor="Crud">{t('crud')}</IGRPLabelPrimitive>
                </div>
                <div className="flex items-center space-x-2">
            ...
            <IGRPTabsPrimitive defaultValue="attributes">
              ...
              {TabList.map(({ value }, key) => (
                <IGRPTabsContentPrimitive key={key} value={value}>
                  <div className="border rounded-lg pb-2">{renderFormList(value)}</div>
                </IGRPTabsContentPrimitive>
              ))}
            </IGRPTabsPrimitive>
          </IGRPCardContentPrimitive>

...
const Overview = () => {
  ...
  const { isGitEnabled, activeBranch } = useSelector((state: RootState) => state.git)
            ...
            <>
              <div className="space-x-3 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  />
                  <div>
                    <h1 className="text-2xl font-semibold">{t('projectDetails')}</h1>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="p-5 rounded-lg border">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-medium">{t('projectInfo')}</h3>
                    <Package className="text-blue-400 w-5 h-5" />
                  </div>
                  {project && (
                    <div className="space-y-4">
                      <div>
                        <p className="text-muted-foreground text-xs mb-1">{t('name')}</p>
                        <p className="truncate">{project.config?.name}</p>
                      </div>
                      <div>
                        <div className="text-muted-foreground">{t('type')}</div>
                        <div className="text-sm font-medium capitalize">{project.type}</div>
                      </div>
                      <div>
                        <p className="text-muted-foreground text-xs mb-1">{t('version')}</p>
                        <p className="">1.0.0</p>
                      </div>
                    </div>
                  )}
                ...
                <div className="p-6 rounded-lg border space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-medium">{t('technicalStack')}</h3>
                    <Server className="text-purple-400 w-5 h-5" />
                  </div>
                  <div className="space-y-4">
                    <div>
                      <p className="text-muted-foreground text-xs mb-1">{t('framework')}</p>
                      <p className="">{project.framework}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground   text-xs mb-1">{t('version')}</p>
                      <p className="">{project.config?.springBootVersion}</p>
                    </div>
                    <Dependency dependsOn={project?.service?.dependsOn} />
                  </div>
                </div>
                <div className="p-6 rounded-lg border space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-medium">{t('development')}</h3>
                    <GitBranchIcon className="text-green-400 w-5 h-5" />
                  </div>
                  <div className="space-y-4">
                    {isGitEnabled && (
                      <div>
                        <p className="text-muted-foreground text-xs mb-1">{t('branch')}</p>
                        <p className="">{activeBranch}</p>
                      </div>
                    )}
                    {repositoryUrl && (
                      <div>
                        <div className="text-muted-foreground text-xs">{t('repository')}</div>
                    ...
                    <GitContributors projectPath={basePath} />
                  </div>
                </div>
              </div>
            ...
            <IGRPCardContentPrimitive className="compact-card-content space-y-3">
              {projectId && (
                <div className="space-y-2">
                  <IGRPLabelPrimitive htmlFor="project-id">{t('projectId')}</IGRPLabelPrimitive>
              ...
              <ProjectConfigForm type={'springboot'} data={data} errors={{}} onChange={setData} />
              <div className="pt-2">
                <IGRPButtonPrimitive onClick={handleSave}>{t('saveSettings')}</IGRPButtonPrimitive>
              </div>
            </IGRPCardContentPrimitive>
          </IGRPCardPrimitive>
        </IGRPTabsContentPrimitive>

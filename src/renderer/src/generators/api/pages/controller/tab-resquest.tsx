...
export const TabRequest: React.FC<TabRequestProps> = ({
  ...
  useEffect(() => {
    ...
    const filteredVariables = newPathVariables.filter((pv) => parameters.includes(pv.name))
  ...
  return (
    <>
      <IGRPTabs defaultValue="params">
        <IGRPTabsList>
          <IGRPTabsTrigger value="params">{t('params')}</IGRPTabsTrigger>
          <IGRPTabsTrigger value="body">{t('body')}</IGRPTabsTrigger>
          <IGRPTabsTrigger value="headers">{t('headers')}</IGRPTabsTrigger>
        </IGRPTabsList>
            ...
            <div className="space-y-3">
              ...
              <div className="border rounded-sm py-0">
                <FormList
                  ...
                  addRow={() => addNewRow(formik, tabQueryParams, initialValues.requestParams[0])}

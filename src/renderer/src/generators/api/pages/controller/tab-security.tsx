...
import { PermissionsConfig } from '@igrp/igrp-studio-springboot-engine/types'
...
export const TabSecurity: React.FC<TabSecurityProps> = ({ formik, basePath, module }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <IGRPCardPrimitive>
        <IGRPCardHeaderPrimitive>
          <IGRPCardTitlePrimitive>{t('permissions') || 'Permissions'}</IGRPCardTitlePrimitive>
          <IGRPCardDescriptionPrimitive>
        ...
        <IGRPCardContentPrimitive className="space-y-4">
          ...
          {permissionItems.length > 0 && (
            <div className="space-y-2">
              ...
              <IGRPCombobox
                options={[
                  { label: `OR ${t('anyPermission') || '(Any permission)'}`, value: 'OR' },
                  { label: `AND ${t('allPermissions') || '(All permissions)'}`, value: 'AND' }
                ]}
                value={permissionOperator}
                onChange={(value) => handlePermissionOperatorChange(value as 'AND' | 'OR')}
          ...
          {/* Permission Input */}
          <div className="space-y-2">
            <IGRPLabelPrimitive>{t('addPermission') || 'Add Permission'}</IGRPLabelPrimitive>
            <div className="flex gap-2">
              {availablePermissions.length > 0 ? (
                ...
                <IGRPInputPrimitive
                  ...
                  placeholder={
                    t('enterPermissionName') || 'Enter permission name (e.g., hr.employee.view)'
                  }
                  className="flex-1"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
              ...
              <IGRPButtonPrimitive
                ...
                disabled={!permissionInput.trim() && availablePermissions.length === 0}
              >
                {t('add') || 'Add'}
              </IGRPButtonPrimitive>
            </div>
          ...
          {permissionItems.length > 0 && (
            <div className="space-y-2">
              <IGRPLabelPrimitive>
                {t('selectedPermissions') || 'Selected Permissions'} ({permissionItems.length})
              </IGRPLabelPrimitive>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <IGRPInputPrimitive
                ...
                placeholder={t('enterRoleName') || 'Enter role name (e.g., ADMIN, USER)'}
                className="flex-1"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault()

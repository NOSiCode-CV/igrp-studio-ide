      ...
      <IGRPPopoverContentPrimitive className="w-[425px]" align="end" side="bottom">
        <IGRPTabsPrimitive defaultValue="dataType">
          <IGRPTabsListPrimitive className="grid w-full grid-cols-2">
            <IGRPTabsTriggerPrimitive value="dataType">{t('dataType')}</IGRPTabsTriggerPrimitive>
          ...
          <IGRPTabsContentPrimitive value="dataType" className="space-y-4">
            <p className="text-sm text-muted-foreground">{t('configureFieldOptions')}</p>
            <div className="flex flex-1 gap-2">
              {['isRequired', 'nullable', 'deprecated'].map((field: string) => (
                <div key={`${field}`} className="flex flex-1 items-center gap-4">
                  <IGRPLabelPrimitive htmlFor={`${field}`}>{toInitCap(field)}</IGRPLabelPrimitive>
                  <IGRPSwitch
                    name={`${field}`}
                    id={`${field}`}
                ...
                {['enum', 'const'].map((field: string) => (
                  <div key={`${field}`} className="flex items-center gap-4">
                    ...
                    <IGRPSwitch
                      name={`${field}`}
                      id={`${field}`}
                      onCheckedChange={(checked) => changeConst(field, checked)}
            ...
            {isInteger && (
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  <IGRPLabelPrimitive htmlFor="minimunm">{t('minimum')}</IGRPLabelPrimitive>
                  <IGRPInputPrimitive
                    id="minimunm"
                    type={'number'}
                    className="h-8 w-28"
                ...
                <div className="space-y-1">
                  <IGRPLabelPrimitive htmlFor="maximum">{t('minimum')}</IGRPLabelPrimitive>
                  <IGRPInputPrimitive
                    id="maximum"
                    type={'number'}
                    className="h-8 w-28"
                ...
                <div className="space-y-1">
                  <IGRPLabelPrimitive htmlFor="m">{t('minimum')}</IGRPLabelPrimitive>
                  <IGRPInputPrimitive
                    id="multipleOf"
                    placeholder=">=0"
                    type={'multipleOf'}
                    ...
                    onChange={(ev) => changeValue('multipleOf', ev.target.value)}
                  />
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-2">
              {!isInteger && !isBoolean && (
                <>
                  <div className="space-y-1">
                    <IGRPLabelPrimitive htmlFor="minLength">{t('minLength')}</IGRPLabelPrimitive>
                    <IGRPInputPrimitive
                      id="minLength"
                      className="h-8"
                      value={row?.['minLength'] || ''}
                      onChange={(ev) => changeValue('minLength', ev.target.value)}
                    />
                  </div>
                  <div className="space-y-1">
                    <IGRPLabelPrimitive htmlFor="maxLength">{t('maxLength')}</IGRPLabelPrimitive>
                    <IGRPInputPrimitive
                      id="maxLength"
                      className="h-8"
                      value={row?.['maxLength'] || ''}
                      onChange={(ev) => changeValue('maxLength', ev.target.value)}
                    />
                  </div>
                </>
              )}
              <div className="space-y-1">
                <IGRPLabelPrimitive htmlFor="default">{t('default')}</IGRPLabelPrimitive>
                <IGRPInputPrimitive
                  id="default"
                  className="h-8"
                  value={row?.['default'] || ''}
              ...
              {!isBoolean && (
                <>
                  <div className="space-y-1">
                    <IGRPLabelPrimitive htmlFor="regex">{t('pattern')}</IGRPLabelPrimitive>
                    <IGRPInputPrimitive
                      id="pattern"
                      className="h-8"
                      value={row?.['pattern'] || ''}
                      onChange={(ev) => changeValue('pattern', ev.target.value)}
                    />
                  </div>

                  <div className="col-span-2 space-y-1">
                    <IGRPLabelPrimitive htmlFor="examples">{t('examples')}</IGRPLabelPrimitive>
                    <IGRPInputPrimitive
                      id="examples"
                      className="h-8"
                      value={row?.['examples'] || ''}
                      onChange={(ev) => changeValue('examples', ev.target.value)}
                    />
                  </div>
                </>
              )}

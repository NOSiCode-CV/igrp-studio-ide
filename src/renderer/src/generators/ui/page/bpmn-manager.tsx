...
import { IGRPButtonPrimitive, useIGRPToast } from '@igrp/igrp-framework-react-design-system'
import {
  IGRPTabsPrimitive,
...
export const BPMNManager = ({
  ...
  const handleDeleteConfig = async (): Promise<void> => {
    ...
    try {
      ...
      // Dispatch custom event to notify other components
      window.dispatchEvent(new CustomEvent('bpmn-config-changed'))
      igrpToast({ type: 'success', content: 'API configuration deleted successfully' })
    } catch (error: unknown) {
  ...
  const loadConfigs = async (): Promise<void> => {
    try {
      const configsData = await window.igrpStudioSettings.getBPMNConfigs()
                ...
                <BPMNConfigCard
                  ...
                  onToggleActive={async (configId: string, isActive: boolean) => {
                    try {
                      ...
                      igrpToast({
                        type: 'success',
                        content: `Configuration ${isActive ? 'activated' : 'deactivated'} successfully`
                      })
                    } catch (error: unknown) {
                  ...
                  onTestConnection={async (config: BPMNConfig) => {
                    try {
                      ...
                      if (result.success) {
                        igrpToast({ type: 'success', content: 'Connection test successful!' })
                      } else {
                        igrpToast({
                          type: 'error',
                          content: `Connection test failed: ${result.message}`
                        })
                      }
                    } catch (error: unknown) {
                      console.error('Error testing connection:', error)
                      igrpToast({ type: 'error', content: 'Connection test failed' })
                    }
                  }}
                />
              ))}

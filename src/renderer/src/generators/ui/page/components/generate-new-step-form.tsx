...
import { getNonVersionFolderItems } from '../utils/bpmn-helpers'
...
export const GenerateNewStepForm: React.FC<GenerateNewStepFormProps> = ({
    ...
    const availableProcesses = useMemo(() => {
        ...
        return bpmnProcesses
            .map((process) => {
                const childrenExcludingVersionFolders = getNonVersionFolderItems(process.children)
                const processConfigFile = childrenExcludingVersionFolders.find(
                    (child) => child.content?.type === 'process'
                )

                if (!processConfigFile) return null

                return {
                    label: `${process.name} - ${processConfigFile.content?.description ?? '—'}`,
                    value: process.name
                }
            })
    ...
    const handleProcessChange = (processName: string): void => {
        ...
        if (process) {
            const availableComponents = process.children
                ?.filter((child) => child.content?.type === 'processStep')
                ?.map((child) => ({
                    label: child.content?.description || child.name,
                    value: child.name
                }))

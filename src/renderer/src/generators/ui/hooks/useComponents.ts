import { Arguments, FieldValidation, State } from '@igrp/igrp-studio-nextjs-engine/dist/interfaces/types';
...
export const useComponents = (): {
    ...
    extractComponentsFromPage: (rootComponent: StructuredComponent, componentName: string) => Map<string, StructuredComponent>;
    ...
    const extractAllStates = useCallback((): State[] => {
            ...
            Object.entries(currentNode?.data || []).forEach(([key, value]) => {
                ...
                // Caso 2: Objeto aninhado que pode conter state
                    ...
                    if ('state' in value && isState(value.state)) {
                        states.push(
                            validateState(value.state, currentNode.tag)
                        );
                    }
                }
            });
        ...
        function isState(obj: any): obj is Partial<State> {
            return (
                obj && typeof obj === 'object' && 'name' in obj && 'type' in obj
            );
        }
    ...
    const extractAllComponentsWithRefs = useCallback((): Map<
        string,
        StructuredComponent
    > => {
        const componentMap = new Map<string, StructuredComponent>();

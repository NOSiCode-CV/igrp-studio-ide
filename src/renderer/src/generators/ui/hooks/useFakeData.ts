...
export const useFakedata = (): {
    ...
    const getDataTableFake = useCallback((columns: StructuredComponent[]) => {
        return Array.from({ length: 4 }).map(() => {
            const rowData: { [key: string]: string | number | boolean | undefined } = {}
            columns.forEach((child: StructuredComponent) => {

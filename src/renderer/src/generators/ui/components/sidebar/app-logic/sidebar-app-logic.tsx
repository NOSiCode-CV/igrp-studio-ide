...
import { ChevronRight, Loader2, LucideWebhook, Workflow, AlertCircle } from 'lucide-react'
...
const SidebarAppLogic = ({ searchTerm = "" }: { searchTerm?: string }) => {
  ...
  useEffect(() => {
    const fetchApplogics = async () => {
      ...
      try {
        console.log(`Fetching applogics: ${endpoint}/${appCode}/endpoints`)
        ...
        const result = await fetchData(`${endpoint}/${appCode}/endpoints`, headers)
        
        if (!result) throw new Error("Failed to fetch applogics")
        if (!Array.isArray(result)) throw new Error("Invalid API response format")

        const processedApplogics = result.map(item => ({
          id: String(item.id) || item.applogicId,
          name: item.name + (item.tag ? ` (${item.tag})` : ''),
          webhooks: Array.isArray(item.nodes) ? item.nodes.map(node => ({
            id: node.resourceId || '',
            path: node.path || '',
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-8 h-full">
        ...
        <p className="text-sm text-muted-foreground">Loading applogics...</p>
      </div>
    )
  }
      ...
      <div className="p-4">
        <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-4 mb-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <h3 className="font-medium text-destructive">Error loading applogics</h3>
              <p className="text-sm text-destructive/80 mt-1">{error}</p>
            </div>
          </div>
        </div>
        <Button onClick={() => setRetryCount(prev => prev + 1)} className="w-full" variant="outline">
          Retry
        </Button>
      </div>
    )
  ...
  if (filteredApplogics.length === 0) {
    return (
      <EmptyList
        title={searchTerm ? "No applogics found" : "No applogics available"}
        description={
          searchTerm
                ...
                <div className="flex items-center">
                  ...
                  <ChevronRight
                    className={`h-4 w-4 transition-transform ${
                      expandedApplogics.includes(workflow.id) ? "rotate-90" : ""
                    }`}
                  />
                </div>
              </SidebarMenuButton>
              ...
              {expandedApplogics.includes(workflow.id) && (
                <SidebarMenuSub>
                  {workflow.webhooks.length > 0 ? (
                    workflow.webhooks.map((webhook, index) => (
                      <SidebarMenuSubItem key={`${webhook.id}-${index}`}>
                        <SidebarMenuSubButton className="flex items-center justify-between gap-2">
                          ...
                          {webhook.method && (
                            <span className={`px-1.5 py-0.5 text-[10px] rounded-full ${getMethodColor(webhook.method)}`}>
                              {webhook.method}
                            </span>
                          )}
                        </SidebarMenuSubButton>

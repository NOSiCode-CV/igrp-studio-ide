              ...
              <IGRPSidebarMenuItemPrimitive key={title}>
                ...
                <IGRPSidebarMenuSubPrimitive>
                  {filteredItems.map((item, index) => {
                    const hasDelete = item.actions ? item.actions.deletable : onDelete && item.id

                    const hasEdit = item.actions ? item.actions.editable : !!item.id
                      ...
                      <IGRPSidebarMenuSubItemPrimitive key={index}>
                        <IGRPSidebarMenuSubButtonPrimitive asChild>
                          <div className="flex items-center justify-between w-full group/item relative">
                            <span className="">{renderItemName(item)}</span>
              ...
              <IGRPDialogTitlePrimitive>
                <div className="flex items-center gap-2 justify-between">
                  <div>
                    ...
                    <span className="text-muted-foreground">{funct ? funct.name : ''}</span>
                  </div>
                  <IGRPButtonPrimitive
                    type="submit"

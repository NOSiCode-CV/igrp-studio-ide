...
import { IGRPCombobox, IGRPOptionsProps } from '@igrp/igrp-framework-react-design-system'
import useCustomCode from '../../../../hooks/useCustomCode'
import { ImportComponent } from '../../../sidebar/custom-code/custom-code-imports'
import { Import, Segment } from '@igrp/igrp-studio-nextjs-engine/types'
...
export function TriggerControls({
  ...
  const [localInteractions, setLocalInteractions] = useState<Record<string, Action>>({})
      ...
      <IGRPDropdownMenuPrimitive>
        ...
        <IGRPDropdownMenuContentPrimitive className="min-w-60">
          {Object.keys(interactionsType).map((key, index) => {
            ...
            return (
              <IGRPDropdownMenuItemPrimitive key={index} onClick={() => addInteraction(key)}>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="space-y-1">
        {localInteractions &&
          Object.entries(localInteractions).map(([key, interaction], index) => {
            return (
              <div
                key={index}
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    <Trash2 size={4} className="text-destructive" />
                  </IGRPButtonPrimitive>
                </div>
              </div>
            )
...
const InteractionEditor = ({
  ...
  const [actionType, setActionType] = useState<ActionType>(interaction.type || 'function')
 ...
 */
  ...
  const hasfnCustomSetOption = properties?.function?.properties.fnCustomSet.visible

  const hasfnCodeOption = properties?.function?.properties.fnCustomCode?.properties?.fnCode?.visible

  const hasImportOption = properties?.function?.properties.fnCustomCode.properties?.imports?.visible
  ...
  const handleChangeFnName = (fnName: string): void => {
    if (fnName) {
      const functionOption = functionOptions.find((option) => option.value === fnName)
          ...
          <div className="space-y-4">
            {hasfnNameOption && (
              <IGRPCombobox
                ...
                onChange={(value) => handleChangeFnName(value as string)}
            ...
            {hasfnCustomSetOption && (
              <>
                <div className="flex-1 border rounded">
                  ...
                  <MonacoEditor
                    content={currentAction.function?.fnCustomSet || ''}
                    filePath=""
                    onChange={(newCode) =>
                      setCurrentAction({
                        ...currentAction,
            ...
            {hasfnCodeOption && (
              <div className="flex-1 border rounded">
                ...
                <MonacoEditor
                  content={currentAction.function?.fnCustomCode?.fnCode || ''}
                  filePath=""
                  onChange={(newCode) =>
                    setCurrentAction({
                      ...currentAction,
        ...
        return (
          <div className="space-y-4">
            <PageSelectionConfig
              ...
              navigationParams={currentAction.navigate?.params || []}
                ...
                if (page) {
                  setCurrentAction({
                    ...
                    navigate: {
                      ...
                      params: currentAction.navigate?.params || []
                    }
                  })
                }
              }}
      ...
      <IGRPDialogContentPrimitive className="p-0 flex overflow-hidden [--header-height:calc(--spacing(99))] [--header-height-three:calc(--spacing(75))] w-full sm:max-w-[800px] lg:max-w-[70vw] max-w-[90vw]">
          ...
          <IGRPDialogHeaderPrimitive className="p-4">
            <div className="flex flex-1 justify-between">
              <div className="space-y-2">
                ...
                <IGRPDialogDescriptionPrimitive>
                  Configure what happens when this interaction is triggered
                </IGRPDialogDescriptionPrimitive>
              </div>
              <div>
                <IGRPButtonPrimitive size={'sm'} onClick={() => saveInteraction()}>
                  Save changes
                </IGRPButtonPrimitive>
              </div>
            </div>
        ...
        {actionType === 'function' && (
          <FunctionSettingsSidebar
            editorRef={hasfnCodeOption ? fnCustomCodeEditorRef : fnCustomSetEditorRef}

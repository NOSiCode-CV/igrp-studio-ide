...
import type { Import, Segment } from '@igrp/igrp-studio-nextjs-engine/types'
import MonacoEditor from '@renderer/components/monaco-editor'
import { useComponents } from '@renderer/generators/ui/hooks/useComponents'
import useStudio from '@renderer/hooks/use-studio'
import { getId } from '@renderer/utils'
import { Edit2, Mouse, Plus, Trash2 } from 'lucide-react'
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent className="min-w-60">
                    {Object.keys(interactionsType).map((key, index) => {
                        ...
                        return (
                            <DropdownMenuItem key={index} onClick={() => addInteraction(key)}>
 ...
 */
    ...
    const hasfnCodeOption =
        properties?.function?.properties.fnCustomCode?.properties?.fnCode?.visible

    const hasImportOption =
        properties?.function?.properties.fnCustomCode.properties?.imports?.visible
                        ...
                        {hasfnCustomSetOption && (
                            <>
                                ...
                                <div className="p-2 text-xs text-muted-foreground border-b bg-muted rounded-t">
                                    Write a custom inline function to execute when the component is
                                    clicked.
                                    <br />
                                    Accepted examples:
                ...
                <SidebarInset className="space-y-4">
                    <DialogHeader className="p-4">
                        <div className="flex flex-1 justify-between">
                            ...
                            <div>
                                <Button size={'sm'} onClick={() => saveInteraction()}>
                                    Save changes
                                </Button>
                            </div>
                        </div>

...
export function TransformControls({ transforms, onTransformsChange }: TransformControlsProps) {
  ...
  const addTransform = () => {
    onTransformsChange([...transforms, { type: 'translate', value: '0', unit: 'px' }])
  }
  ...
  const updateTransform = (index: number, field: keyof TransformValue, value: string) => {
    const newTransforms = [...transforms]
    const transformType = transformTypes.find(
      (t) => t.name === (field === 'type' ? value : newTransforms[index].type)
    )
  ...
  const TransformEditor = ({ transform, index }: { transform: TransformValue; index: number }) => {
    const transformType = transformTypes.find((t) => t.name === transform.type)
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-0.5">
            ...
            <select
              ...
              onChange={(e) => updateTransform(index, 'type', e.target.value)}
          ...
          <div className="space-y-0.5">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                onChange={(e) => updateTransform(index, 'value', e.target.value)}
              ...
              <select
                ...
                onChange={(e) => updateTransform(index, 'unit', e.target.value)}
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <div className="relative">
                ...
                {editingIndex === index && <TransformEditor transform={transform} index={index} />}
              </div>
              <button
                onClick={() => removeTransform(index)}
                className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded"

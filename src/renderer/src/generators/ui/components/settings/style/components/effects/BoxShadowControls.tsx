import { BoxSelect, Edit2, Lock, Plus, Trash2, Unlock } from 'lucide-react'
...
export function BoxShadowControls({
    ...
    const updateShadow = (index: number, field: keyof ShadowValue, value: any) => {
    ...
    const ShadowEditor = ({ shadow, index }: { shadow: ShadowValue; index: number }) => (
        <div
            ref={popoverRef}
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center">
                    <label className="flex items-center gap-1.5">
                        ...
                        />
                        <span className="text-xs text-gray-500">{t('insideShadow')}</span>
                    </label>
                </div>
                <div className="grid grid-cols-2 gap-1">
                    <div className="space-y-0.5">
                        <label className="text-xs text-gray-500">{t('offsetX')}</label>
                        <input
                            type="number"
                            value={shadow.x}
                            onChange={(e) => updateShadow(index, 'x', e.target.value)}
                    ...
                    <div className="space-y-0.5">
                        <label className="text-xs text-gray-500">{t('offsetY')}</label>
                        <input
                            type="number"
                            value={shadow.y}
                            onChange={(e) => updateShadow(index, 'y', e.target.value)}
                            className="w-full px-1.5 py-0.5 text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded border-0 focus:ring-2 focus:ring-blue-500"
                        />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-1">
                    <div className="space-y-0.5">
                        ...
                        <input
                            ...
                            min="0"
                            value={shadow.blur}
                            onChange={(e) => updateShadow(index, 'blur', e.target.value)}
                            className="w-full px-1.5 py-0.5 text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded border-0 focus:ring-2 focus:ring-blue-500"
                        />
                    </div>
                    <div className="space-y-0.5">
                        <label className="text-xs text-gray-500">{t('spread')}</label>
                        <input
                            type="number"
                            value={shadow.spread}
                            onChange={(e) => updateShadow(index, 'spread', e.target.value)}
                            className="w-full px-1.5 py-0.5 text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded border-0 focus:ring-2 focus:ring-blue-500"
                        />
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        title={linkedShadow ? 'Unlink shadow values' : 'Link shadow values'}
                    >
                        {linkedShadow ? <Lock size={10} /> : <Unlock size={10} />}
                    </button>
                    <button
                        onClick={addShadow}
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="text-xs font-medium text-gray-700 dark:text-gray-300">
                                {shadow.inset ? 'Inside' : 'Outside'} Shadow {index + 1}
                            </div>
                        ...
                        <div className="flex items-center gap-1">
                            <div className="relative">
                                ...
                                {editingIndex === index && (
                                    <ShadowEditor shadow={shadow} index={index} />
                                )}
                            </div>
                            <button
                                onClick={() => removeShadow(index)}

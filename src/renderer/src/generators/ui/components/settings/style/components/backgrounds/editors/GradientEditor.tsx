...
export function GradientEditor({ background, onChange }: GradientEditorProps) {
    ...
    const addStop = () => {
        ...
        const position = Math.min(
            100,
            Number(lastStop.position) +
                (Number(lastStop.position) - Number(secondLastStop.position))
        );
            ...
            <div className="grid grid-cols-2 gap-1">
                ...
                <div className="space-y-0.5">
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            updateGradient({ angle: e.target.value })
                        }
                        min="0"
                        max="360"
                        className="w-full px-1.5 py-0.5 text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded border-0 focus:ring-2 focus:ring-blue-500"
                ...
                {gradient.stops.map((stop, index) => (
                    <div key={index} className="flex items-center gap-1">
                        <input
                            ...
                            value={stop.color}
                            onChange={(e) =>
                                updateStop(index, e.target.value, stop.position)
                            }
                        ...
                        <input
                            ...
                            value={stop.position}
                            onChange={(e) =>
                                updateStop(index, stop.color, e.target.value)
                            }
                            min="0"
                            max="100"

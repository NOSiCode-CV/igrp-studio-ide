...
export function layoutStyleToClasses(layout: LayoutStyle): string {
    ...
    if (
        layout.flex &&
        (layout.type === 'flex' || layout.type === 'inline-flex')
    ) {
        const { direction, wrap, alignItems, justifyContent, gap } =
            layout.flex;
    ...
    if (
        layout.grid &&
        (layout.type === 'grid' || layout.type === 'inline-grid')
    ) {
        classes.push('grid');
        ...
        if (
            layout.grid.gap &&
            !isNaN(Number(layout.grid.gap.replace('px', '')))
        ) {
        ...
        // Grid alignment
        classes.push(
            layout.grid.justifyItems === 'start'
                ? 'justify-items-start'
                : layout.grid.justifyItems === 'end'
                  ? 'justify-items-end'
                  : layout.grid.justifyItems === 'center'
                    ? 'justify-items-center'
                    : layout.grid.justifyItems === 'stretch'
                      ? 'justify-items-stretch'
                      : ''
        );

        classes.push(
            layout.grid.alignItems === 'start'
                ? 'items-start'
                : layout.grid.alignItems === 'end'
                  ? 'items-end'
                  : layout.grid.alignItems === 'center'
                    ? 'items-center'
                    : layout.grid.alignItems === 'stretch'
                      ? 'items-stretch'
                      : ''
        );
    }

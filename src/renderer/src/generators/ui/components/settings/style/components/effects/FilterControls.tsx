...
import { type FilterValue, filterTypes } from './types'
...
export function FilterControls({ title, filters, onFiltersChange }: FilterControlsProps) {
    ...
    const updateFilter = (index: number, field: keyof FilterValue, value: string) => {
        ...
        const filterType = filterTypes.find(
            (t) => t.name === (field === 'type' ? value : newFilters[index].type)
        )
                    ...
                    >
                        ...
                        <div className="flex items-center gap-1">
                            <div className="relative">
                                ...
                                {editingIndex === index && (
                                    <FilterEditor filter={filter} index={index} />
                                )}
                            </div>
                            <button
                                onClick={() => removeFilter(index)}

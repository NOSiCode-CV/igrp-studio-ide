...
export function LayoutSection() {
  ...
  const additionalOptions = [
    { value: 'Inline-block', label: 'Inline-block', icon: <Square size={14} /> },
    { value: 'Inline-flex', label: 'Inline-flex', icon: <AlignCenter size={14} /> },
    { value: 'Inline-grid', label: 'Inline-grid', icon: <LayoutGrid size={14} /> },
    { value: 'Inline', label: 'Inline', icon: <AlignCenter size={14} /> },
  ];

  const allOptions = [...mainOptions, ...additionalOptions];
  const selectedOption = allOptions.find(opt => opt.value === selectedDisplay);
  const isAdditionalOptionSelected = additionalOptions.some(opt => opt.value === selectedDisplay);
  const isFlexDisplay = selectedDisplay === 'Flex' || selectedDisplay === 'Inline-flex';
  const isGridDisplay = selectedDisplay === 'Grid' || selectedDisplay === 'Inline-grid';
  ...
  return (
    <div className="space-y-2">
      <div className="space-y-0.5 relative">
        <label className="text-[10px] text-gray-500 dark:text-gray-400">Display</label>
          ...
          {mainOptions.map((option) => (
            <button
              ...
              className={`
                ...
                text-[10px] transition-all duration-200 transform
                ${
              ...
              `}
            >
              <div className={`mb-0.5 transition-transform duration-200 ${selectedDisplay === option.value ? 'scale-110' : ''}`}>
                {option.icon}
              </div>
              ...
              <div className="absolute z-20 w-full mt-0.5 py-0.5 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700">
                {additionalOptions.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => {
                      setSelectedDisplay(option.value);
                      setIsOpen(false);
                    }}
                    className={`
                    ...
                    `}
                  >
                    <div className={`transition-transform duration-200 ${selectedDisplay === option.value ? 'scale-110' : ''}`}>
                      {option.icon}
                    </div>
                    <span className="font-medium">{option.label}</span>
                  </button>

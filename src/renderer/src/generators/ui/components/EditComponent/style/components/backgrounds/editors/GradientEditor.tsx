...
export function GradientEditor({ background, onChange }: GradientEditorProps) {
  ...
  const addStop = () => {
    ...
    const position = Math.min(100, Number(lastStop.position) + (Number(lastStop.position) - Number(secondLastStop.position)));
    ...
    <div className="space-y-2">
      <div className="grid grid-cols-2 gap-1">
        <div className="space-y-0.5">
          ...
          <select
            value={gradient.type}
            onChange={(e) => updateGradient({ type: e.target.value as 'linear' | 'radial' | 'conic' })}
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <label className="text-[9px] text-gray-500">Color Stops</label>
          <button
            onClick={addStop}
            disabled={gradient.stops.length >= 5}
            className="text-[9px] text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 disabled:opacity-50"
          >
            Add Stop
          </button>
        </div>

...
import { Destination, DragEndResult, StructuredComponent } from '@renderer/lib/dnd/types'
...
export const AddComponentModal = ({ path, comp, parentComp, open, setOpen }: AddComponentProps) => {
  ...
  const onEditComponent = (component: StructuredComponent, parentComponent?: string) => {
    ...
    setCurrentPath(`${path}/${comp.componentName}${parentComponent ? '/' + parentComponent : ''}`)
  }
    ...
    <>
      <IGRPDialogPrimitive open={open} onOpenChange={setOpen}>
        <IGRPDialogContentPrimitive className="p-0 flex overflow-hidden [--header-height-three:calc(--spacing(75))] !max-w-[80vw] !h-[80vh]">
          <IGRPSidebarInsetPrimitive>
            ...
            <div className="flex-1 overflow-auto p-4">
              ...
              <div className="p-2 text-xs text-muted-foreground border-b bg-muted rounded-t mt-6">
                <p className="mb-2">
                  <strong>rowData</strong> is a variable provided by the table that contains all the
                  data from the current row. Use it in your click handlers to access row
                  information.
                </p>
          ...
          <SidebarRight comp={currentComponent} path={currentPath} parentComp={parentComp} />
        </IGRPDialogContentPrimitive>
      </IGRPDialogPrimitive>
    </>
  )
...
const RenderCreatedComponents = ({
  ...
  const canAcceptChildren = (component: StructuredComponent) => {
    const registryComponent = registryComponents.find((rc) => rc.name === component.componentName)

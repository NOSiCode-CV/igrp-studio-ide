...
export const DroppedComponentsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    (
      ...
      const updateComponentTree = (component: StructuredComponent): StructuredComponent => {
        ...
        if (component.children) {
          const updatedChildren = component.children.map(updateComponentTree)
          return {
            ...component,
            children: updatedChildren
          }
  ...
  const handleReorderChildInComponent = useCallback(
    (_draggableId: string, source: Destination, destination: Destination) => {
      ...
      const removeComponentFromSource = (component: StructuredComponent): StructuredComponent => {
  ...
  const handleRemoveChildFromComponent = useCallback((destination: Destination) => {
    ...
    const updateComponentTree = (component: StructuredComponent): StructuredComponent | null => {
  ...
  const removeRow = useCallback((rowId: string) => {
    setComponents((prevComponents) => {
      const updatedComponents = {
        ...prevComponents,
        children: (prevComponents.children || []).filter((row) => row.id !== rowId)
      }
  ...
  const setEditingComponent = ({ path, component }: EditingComponentParams) => {
  ...
  const updateType = (id: string, updates: Partial<TypeDef>) => {
    setTypes((prev) =>
      prev.map((type) => (type.componentId === id ? { ...type, ...updates } : type))
    )
  }
  ...
  const createOrUpdateType = (newType: TypeDef) => {
    setTypes((prevTypes) => {
      const existingIndex = prevTypes.findIndex((t) => t.componentId === newType.componentId)
  ...
  const updateFunction = (id: string, updates: Partial<CustomFunctionConfig>) => {
    setFunctions((prev) => prev.map((fnc) => (fnc.id === id ? { ...fnc, ...updates } : fnc)))
  }
  ...
  const updateImport = (id: string, updates: Partial<Import>) => {
    setImports((prev) =>
      prev.map((importPath) => (importPath.id === id ? { ...importPath, ...updates } : importPath))
    )
  }

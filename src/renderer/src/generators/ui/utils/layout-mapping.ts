...
export const layoutMapping = {
    ...
    grid: {
        ...
        'responsive-cols-small':
            'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4',
        'responsive-cols-large':
            'grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4',
        ...
        // Common layouts
        'holy-grail':
            'grid grid-cols-[200px_1fr_200px] grid-rows-[auto_1fr_auto] min-h-screen gap-4',
        'sidebar-left': 'grid grid-cols-[250px_1fr] gap-4',
        'sidebar-right': 'grid grid-cols-[1fr_250px] gap-4',
        'header-content-footer':
            'grid grid-rows-[auto_1fr_auto] min-h-screen gap-4',
...
export const formVariants = cva('flex flex-col gap-y-8', {
    variants: {
        variant: layoutMapping['grid'],
    },
    defaultVariants: {
...
export const flexVariants = cva('flex flex-1 gap-4', {
    variants: {
    ...
    Object.entries(variantObj).forEach(([breakpoint, value]) => {
        if (value && typeof value === 'string') {
            ...
            if (value.startsWith('cols')) {
                ...
                value.includes('grid-cols-') ||
                value.includes('grid-rows-') ||
                value.includes('col-span-')
            ) {
...
export const useResponsiveClasses = (
    ...
    const finalClasses = [baseClass, responsiveClasses, additionalClasses]
        .filter(Boolean)
        .join(' ');

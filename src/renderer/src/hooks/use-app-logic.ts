...
export function useAppLogic() {
        ...
        const initialize = async () => {
            try {
                ...
                const storedEnvironments =
                    await AppLogicIPCClient.getEnvironments();
                ...
                // Real-time updates
                const unsubscribe = AppLogicIPCClient.onEnvironmentsChanged(
                    (newEnvironments) => {
    ...
    const createEnvironment = useCallback(
        async (
            ...
            try {
                const validation =
                    EnvironmentValidator.validateEnvironment(data);
                ...
                const sanitized =
                    EnvironmentValidator.sanitizeEnvironment(data);
                ...
                const created =
                    await AppLogicIPCClient.addEnvironment(environment);
                toast.success(
                    `Environment "${created.name}" created successfully`
                );
                return created;
            } catch (err) {
    ...
    const testEnvironment = useCallback(
        async (id: string) => {
            ...
            try {
                ...
                const result =
                    await window.appLogicAPI.testEnvironment(environment);
                console.log(result);
                ...
                if (result.isValid) {
                    toast.success(
                        `Environment tested successfully (${result.responseTime}ms)`
                    );
                } else {
    ...
    return {
        ...
        searchEnvironments,
        exportEnvironments,
        getEnvironmentHistory: (id: string) =>
            AppLogicIPCClient.getEnvironmentHistory(id),
    };
}

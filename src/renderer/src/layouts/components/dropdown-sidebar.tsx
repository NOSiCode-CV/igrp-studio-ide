      ...
      <IGRPDropdownMenuPrimitive>
        <IGRPDropdownMenuTriggerPrimitive asChild>
          <div className="text-muted-foreground hover:text-foreground">
            {isDeleteAction ? <Ellipsis className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
          </div>
        </IGRPDropdownMenuTriggerPrimitive>
          ...
          {(menuItem.dropdownMenus ?? []).map((menu: DropdownItem, idx: number) => {
            ...
            return (
              <React.Fragment key={idx}>
                {menu.actionType === OPTION_TYPE.DELETE && <IGRPDropdownMenuSeparatorPrimitive />}
                <IGRPDropdownMenuItemPrimitive
                  onClick={(e) => {
                    e.stopPropagation()
                    handleDropdownClick({
                ...
                >
                  {menu.icon ? <menu.icon className={'h-4'} /> : <span className="h-4 me-4"></span>}
                  {menu.label}
                  {isDelete && (
                    <IGRPDropdownMenuShortcutPrimitive>⌘+D</IGRPDropdownMenuShortcutPrimitive>
                  )}
                </IGRPDropdownMenuItemPrimitive>
              </React.Fragment>
            )

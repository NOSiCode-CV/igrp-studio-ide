    ...
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="flex gap-2 p-2 border-b">
        <select
          value={filter}
          onChange={(e) =>
            setFilter(e.target.value as 'all' | 'info' | 'warn' | 'error' | 'debug' | 'success')
          }
        ...
        <IGRPButtonPrimitive
          ...
          size="icon"
          onClick={() => {
            navigator.clipboard.writeText(filteredLogs.map((log) => log.message).join('\n'))
          }}
              ...
              <div className="space-y-1">
                {filteredLogs.map((log, index) => (
                  ...
                  >
                    ...
                    <div className={getLogLevelColor(log.level)}>{log.message}</div>
                  </div>
                ))}
              </div>
            ) : (

                  ...
                  <IGRPContextMenuPrimitive>
                    <IGRPContextMenuTriggerPrimitive>
                      <div
                        className={cn(
                          ...
                          {
                            'text-primary border-t-2 border-primary': activeTab === tab.id
                          }
                        )}
                        onClick={() => setActiveTab(tab.id)}
                      >
                        <div className="flex items-center space-x-1 group/tab">
                          {tab.item && tab.item.badgeName ? (
                            <span className={cn('text-orange-500', tab.item.badgeColor)}>
                              {tab.item.badgeName}
                            </span>
                          ) : tab.id === TAB_DEFAULT ? (
                            <Book className="h-3" />
                          ...
                          {tab.id !== TAB_DEFAULT && (
                            <IGRPButtonPrimitive
                              ...
                              variant="ghost"
                              size="sm"
                              className={cn('opacity-0 group-hover/tab:opacity-100 size-5')}
                            >
                              <X className="h-3" />
                            </IGRPButtonPrimitive>
                          )}
                    ...
                    {tab.id !== TAB_DEFAULT && (
                      <IGRPContextMenuContentPrimitive className="w-64">
                        <IGRPContextMenuItemPrimitive onClick={() => handleCloseTab(tab.id)}>
                          {t('closeSelectedTab')}
                          <IGRPContextMenuShortcutPrimitive>
                            {SHORTCUTS.CLOSE_TAB}
                          </IGRPContextMenuShortcutPrimitive>
                        </IGRPContextMenuItemPrimitive>
                        <IGRPContextMenuItemPrimitive onClick={() => handleCloseRight(tab.id)}>
                          {t('closeRight')}
                        </IGRPContextMenuItemPrimitive>
                        <IGRPContextMenuItemPrimitive onClick={() => handleCloseLeft(tab.id)}>
                          {t('closeLeft')}
                        </IGRPContextMenuItemPrimitive>
                        <IGRPContextMenuItemPrimitive onClick={() => handleCloseOthers(tab.id)}>
                          {t('closeOthers')}
                        </IGRPContextMenuItemPrimitive>
                        <IGRPContextMenuItemPrimitive onClick={handleCloseAll}>
                          {t('closeAll')}
                        </IGRPContextMenuItemPrimitive>
                      </IGRPContextMenuContentPrimitive>
                    )}
                  </IGRPContextMenuPrimitive>
                  <IGRPSeparatorPrimitive orientation="vertical" className="h-4" />
                </React.Fragment>
              )
            })}
          </div>

  ...
  return (
    <IGRPDropdownMenuPrimitive>
      <IGRPDropdownMenuTriggerPrimitive asChild>
        <IGRPButtonPrimitive variant={variant} className={cn('h-6 px-2 text-sm', className)}>
                        ...
                        <div className="max-h-[60svh]">
                          {Object.entries(items).map(([category, subItems]) => (
                            <React.Fragment key={category}>
                              <IGRPDropdownMenuItemPrimitive disabled>
                                {getLabel(category)}
                              </IGRPDropdownMenuItemPrimitive>
                              {(subItems as string[]).map((subItemValue, key) => (
                                <IGRPDropdownMenuItemPrimitive
                                  key={key}
                                  onClick={() =>
                                    onTypeChange({

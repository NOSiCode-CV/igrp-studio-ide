...
import { Check, ChevronsUpDown, PlusCircle, GitBranch, GitFork } from 'lucide-react'
...
export function BranchSwitcher({
  ...
  const { isGitEnabled, branches, activeBranch } = useSelector((state: RootState) => state.git)
  ...
  const handleInitGit = async () => {
    try {
      await window.electron.ipcRenderer.invoke('initialize-git', projectPath)
    ...
    return (
      <IGRPTooltipPrimitive>
        <IGRPTooltipTriggerPrimitive asChild>
          <IGRPButtonPrimitive variant={'outline'} onClick={handleInitGit} className="h-6 text-xs">
              ...
              {branches.map((branch) => (
                ...
                >
                  <div className="flex items-center">
                    <Check
                      className={cn('mr-2 h-4 w-4', branch.isActive ? 'opacity-100' : 'opacity-0')}
                    />

...
import { GitFork, ExternalLink, FolderOpen, Calendar, Lock, AlertCircle } from 'lucide-react'
import {
  IGRPButtonPrimitive,
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      if (!folderExists || !config) {
        await window.electron.ipcRenderer.invoke('remove-cloned-repo', repo.id)
    ...
    <IGRPTooltipProviderPrimitive>
      <div className="flex items-center justify-between py-3 px-4 hover:bg-muted transition-colors">
              ...
              <div className="flex items-center gap-2">
                <IGRPTooltipPrimitive>
                  <IGRPTooltipTriggerPrimitive asChild>
                    <h3 className="font-medium text-base truncate">{repo.full_name}</h3>
                  </IGRPTooltipTriggerPrimitive>
                  <IGRPTooltipContentPrimitive> {repo.full_name}</IGRPTooltipContentPrimitive>
                </IGRPTooltipPrimitive>
                {repo.private && (
                  <IGRPTooltipPrimitive>
                    <IGRPTooltipTriggerPrimitive asChild>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {isCloned ? (
            <IGRPButtonPrimitive size="sm" variant="secondary" onClick={handleOpen}>

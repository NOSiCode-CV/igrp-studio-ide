...
export default function DynamicKeyValueForm({
  ...
  const validateItems = (): boolean => {
    ...
    items.forEach((item) => {
      const missingFields = fieldPairs.filter((pair) => !item[pair.key]).map((pair) => pair.label)
    ...
    <div className="w-full border bg-card rounded-lg space-y-6 px-2">
      <IGRPTablePrimitive>
        <IGRPTableHeaderPrimitive>
          <IGRPTableRowPrimitive>
            {fieldPairs.map((pair) => (
              <IGRPTableHeadPrimitive key={pair.key}>{t(pair.label)}</IGRPTableHeadPrimitive>
            ))}
            <IGRPTableHeadPrimitive>
              <IGRPButtonPrimitive
                onClick={addItem}
                ...
                <IGRPTableCellPrimitive key={`${item.id}-${pair.key}`}>
                  <div className="flex flex-col">
                    {pair.options && pair.options.length > 0 ? (
                      <IGRPCombobox
                        value={item[pair.key] || ''}
                        onChange={(val) => updateItem(item.id, pair.key, val as string)}
                      ...
                      <IGRPInputPrimitive
                        value={item[pair.key] || ''}
                        onChange={(e) => updateItem(item.id, pair.key, e.target.value)}
                        placeholder={pair.placeholder ?? `Enter ${pair.label}`}

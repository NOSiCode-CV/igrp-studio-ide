...
interface CloneProjectModalProps {
  handleCloneProject: (url: string, name: string, location: string, auth: { type: string; username?: string; password?: string; token?: string }) => void
  children: React.ReactNode
}

export function CloneProjectModal({ handleCloneProject, children }: CloneProjectModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] bg-background text-foreground">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">Clone Project</DialogTitle>
        </DialogHeader>
        <div className="grid gap-6 py-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2 grid gap-2">
              <Label htmlFor="project-url" className="text-muted-foreground">Repository URL</Label>
              <Input
                id="project-url"
            ...
            <div className="grid gap-2">
              <Label htmlFor="clone-location" className="text-muted-foreground">Clone Location</Label>
              <Select value={cloneLocation} onValueChange={setCloneLocation}>
                <SelectTrigger id="clone-location" className="bg-background text-foreground">
            ...
            <Tabs value={authType} onValueChange={setAuthType}>
              ...
              <TabsContent value="token">
                <div className="grid gap-2 mt-4">
                  <Label htmlFor="token" className="text-muted-foreground">Access Token</Label>
                  <div className="relative">
                    <Key className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                    <Input
                      id="token"

...
import { IGRPInputPrimitive, IGRPSwitchPrimitive } from '@igrp/igrp-framework-react-design-system'
...
export default function KeyboardShortcuts() {
  ...
  const renderShortcut = (shortcut: string | string[]) => {
    if (Array.isArray(shortcut)) {
      return (
        <>
          <kbd className="rounded bg-muted px-2 py-1 text-xs">{shortcut[0]}</kbd>
          <span className="text-xs text-muted-foreground">{t('or')}</span>
          <kbd className="rounded bg-muted px-2 py-1 text-xs">{shortcut[1]}</kbd>
        </>
      )
    }
    return <kbd className="rounded bg-muted px-2 py-1 text-xs">{shortcut}</kbd>
  }
      ...
      <div className="pb-4 space-y-4">
        <div className="flex items-center justify-between py-4">
          <div className="flex items-center space-x-2">
            <span className="text-sm text-muted-foreground">{t('keyboardShortcuts')}</span>
            ...
            <div className="mt-2 space-y-2">
              {group.shortcuts.map((shortcut) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">{renderShortcut(shortcut.shortcut)}</div>
                </div>
              ))}
            </div>
          </div>

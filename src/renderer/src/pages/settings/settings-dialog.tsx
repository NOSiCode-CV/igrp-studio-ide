    ...
    <IGRPDialogPrimitive open={open} onOpenChange={setOpen}>
      ...
      <IGRPDialogContentPrimitive className="overflow-hidden p-0 md:max-h-[500px] sm:max-w-[600px] md:max-w-[700px] max-w-4xl">
        <IGRPDialogHeaderPrimitive className="pb-3">
          <IGRPDialogTitlePrimitive className="sr-only">{t('settings')}</IGRPDialogTitlePrimitive>
            ...
            <IGRPScrollAreaPrimitive className="h-full">
                ...
                <div className="flex items-center gap-2 px-4">
                  <IGRPBreadcrumbPrimitive>
                    <IGRPBreadcrumbListPrimitive>
                      ...
                      <IGRPBreadcrumbItemPrimitive>
                        <IGRPBreadcrumbPagePrimitive>{t(activeItem)}</IGRPBreadcrumbPagePrimitive>
                      </IGRPBreadcrumbItemPrimitive>
                    </IGRPBreadcrumbListPrimitive>
                  </IGRPBreadcrumbPrimitive>
                </div>
              ...
              <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-4 pt-0">
                {data.nav.find((item) => item.name === activeItem)?.component ? (
                  React.createElement(data.nav.find((item) => item.name === activeItem)!.component!)
                ) : (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-muted-foreground">{t('selectSettingToView')}</p>
                  </div>
                )}
              </div>

    ...
    <div className="space-y-4">
      <IGRPCard>
        <IGRPCardHeader className="compact-card-header">
          <IGRPCardTitle className="text-sm">{t('workspaceInformation')}</IGRPCardTitle>
          <IGRPCardDescription className="text-xs">{t('workspaceInformation')}</IGRPCardDescription>
        </IGRPCardHeader>
        <IGRPCardContent className="compact-card-content space-y-3">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground mt-1">{t('workspaceIdDescription')}</p>
          </div>

          <div className="space-y-2">
            <IGRPLabelPrimitive htmlFor="workspace-name">{t('nameDescription')}</IGRPLabelPrimitive>
            <IGRPInputPrimitive
              id="workspace-name"
              value={workspaceName}
        ...
        <IGRPCardFooter className="compact-card-footer flex justify-between">
          ...
          <IGRPButtonPrimitive
            ...
            disabled={
              isSaving ||
              (workspaceName === workspace.name && workspaceDescription === workspace.description)
            }
          >
        ...
        <IGRPCardContent className="compact-card-content space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-muted-foreground">{t('autoSaveChanges')}</div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-medium">{t('enableVersioning')}</div>
              <div className="text-sm text-muted-foreground">{t('trackChanges')}</div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-medium">{t('experimentalFeatures')}</div>
              <div className="text-sm text-muted-foreground">{t('enableExperimentalFeatures')}</div>
            </div>
      ...
      <IGRPCard className="border-destructive/50">
        <IGRPCardHeader className="compact-card-header">
          <IGRPCardTitle className="text-sm text-destructive">{t('dangerZone')}</IGRPCardTitle>
          <IGRPCardDescription className="text-xs">{t('irreversibleActions')}</IGRPCardDescription>
        </IGRPCardHeader>
        <IGRPCardContent className="compact-card-content">
          <div className="border rounded-md border-destructive/30 p-3">
            <div className="flex items-start justify-between">
              <div>
                <h4 className="text-xs font-medium">{t('deleteWorkspace')}</h4>
                <p className="text-sm text-muted-foreground">{t('deleteWarning')}</p>
              </div>
              <IGRPModalDialog>
                <IGRPModalDialogTrigger asChild>
                  <IGRPButtonPrimitive variant="destructive" size="sm" className="h-7">
                ...
                <IGRPModalDialogContent className="compact-dialog">
                  ...
                  <div className="py-3">
                    <div className="border rounded-md p-2 bg-muted/30">
                      <div className="text-xs font-medium mb-1">{workspace.name}</div>
                  ...
                  <IGRPModalDialogFooter className="compact-dialog-footer flex flex-1 items-center ">
                    <IGRPButtonPrimitive variant="outline" className="h-7 text-xs">
                      {t('cancel')}
                    </IGRPButtonPrimitive>
                    <IGRPButtonPrimitive

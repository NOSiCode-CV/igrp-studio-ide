...
export const PortsBadgeList = ({
  ...
  if (ports.length === 0) {
    return <div className={cn('p-1 text-xs text-muted-foreground', className)}>{emptyMessage}</div>
  }
    ...
    <div className={cn('py-1', className)}>
      ...
      <div className="text-xs font-mono flex flex-wrap gap-1 mt-0.5">
        {ports.map((port, index) => (
          <IGRPBadgePrimitive key={`${port}-${index}`} variant="outline" className={badgeClassName}>
            {port}
          </IGRPBadgePrimitive>
        ))}
      </div>

...
import { FolderKanban, Pin, Plus, FolderOpen, EllipsisVertical } from 'lucide-react'
        ...
        <IGRPSidebarGroupPrimitive className="p-0">
          <IGRPSidebarMenuPrimitive className="gap-1">
              ...
              >
                ...
                {filteredWorkspaces.map((workspace, index) => (
                  <IGRPDropdownMenuItemPrimitive
                    ...
                    className={cn(
                      workspace.id === selectedWorkspace.id && 'text-primary bg-primary/5'
                    )}
                  >
                    {workspace.name}
                    ...
                    <IGRPDropdownMenuShortcutPrimitive>
                      {index < 9
                        ? SHORTCUTS[`SWITCH_WORKSPACE_${index + 1}` as keyof typeof SHORTCUTS]
                        : ''}
                    </IGRPDropdownMenuShortcutPrimitive>
                  </IGRPDropdownMenuItemPrimitive>
                ))}
                ...
                >
                  ...
                  <div className="font-medium text-muted-foreground">{t('openWorkspace')}</div>
                  <IGRPDropdownMenuShortcutPrimitive>
                ...
                >
                  ...
                  <div className="font-medium text-muted-foreground">{t('addWorkspace')}</div>
                  <IGRPDropdownMenuShortcutPrimitive>
            ...
            {pinnedWorkspaces.map((workspace) => (
              <IGRPSidebarMenuItemPrimitive key={workspace.id} className="ml-3 border-l-0">
                <IGRPSidebarMenuButtonPrimitive
                  asChild
                  className={cn(workspace.id === selectedWorkspace.id && 'bg-muted')}
                  ...
                  <div className="flex items-center justify-between pl-9 cursor-pointer">
                    ...
                    <IGRPTooltipPrimitive>
                      <IGRPTooltipTriggerPrimitive asChild>
                        ...
                        >
                          <Pin className={cn('h-3 w-3', workspace.pinned && 'text-primary')} />
                        </IGRPButtonPrimitive>
                      </IGRPTooltipTriggerPrimitive>
                      <IGRPTooltipContentPrimitive>
                        {workspace.pinned ? t('unpinWorkspace') : t('pinWorkspace')}
                      </IGRPTooltipContentPrimitive>
                    </IGRPTooltipPrimitive>
                  </div>
                </IGRPSidebarMenuButtonPrimitive>

  ...
  const fetchTemplates = async () => {
    ...
    const convertedTemplates =
      result.services?.map((service: any) => {
        ...
        const volumes =
          serviveData.volumes?.map((vol: any) => `${vol.name}:${vol.path}:${vol.driver}`) || []

        // Convert dependsOn to depends_on array
        const dependsOn =
          serviveData.dependsOn?.map((dep: any) => (typeof dep === 'string' ? dep : dep.service)) ||
          []
  ...
  return (
    <IGRPDialogPrimitive open={open} onOpenChange={setOpen}>
      <IGRPDialogContentPrimitive className="sm:max-w-2xl max-h-[70vh] overflow-hidden flex flex-col">
        ...
        <IGRPTabsPrimitive defaultValue="basic" className="flex-1 overflow-hidden flex flex-col">
          <IGRPTabsListPrimitive className="grid grid-cols-4 mb-4 w-full">
            ...
            <IGRPTabsTriggerPrimitive value="config">{t('configuration')}</IGRPTabsTriggerPrimitive>
            <IGRPTabsTriggerPrimitive value="dependencies">
              {t('dependencies')}
            </IGRPTabsTriggerPrimitive>
            <IGRPTabsTriggerPrimitive value="network">{t('network')}</IGRPTabsTriggerPrimitive>
          </IGRPTabsListPrimitive>
          ...
          <div className="pb-4">
            <IGRPTabsContentPrimitive value="basic" className="mt-0 space-y-4 px-1">
              ...
              <div className="space-y-2">
                <IGRPLabelPrimitive htmlFor="name">{t('serviceName')}</IGRPLabelPrimitive>
                <IGRPInputPrimitive
                  id="name"
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <IGRPLabelPrimitive htmlFor="image">{t('dockerImage')}</IGRPLabelPrimitive>
                  <IGRPInputPrimitive
                    id="image"
                    value={image}
                ...
                <div className="space-y-2">
                  <IGRPLabelPrimitive htmlFor="type">{t('serviceType')}</IGRPLabelPrimitive>
                  <IGRPCombobox
                    value={type}
            ...
            <IGRPTabsContentPrimitive value="config" className="mt-0 space-y-4 flex-1 min-h-0">
              <IGRPScrollAreaPrimitive className="h-[400px]">
                ...
                <div className="border rounded-md p-3 space-y-2 mb-3">
                  ...
                  <p className="text-xs text-muted-foreground">{t('portsFormat')}</p>
                </div>

                <div className="space-y-2 mb-3">
                  <div className="flex items-center justify-between">
                    <IGRPLabelPrimitive>{t('environmentVariables')}</IGRPLabelPrimitive>
                    <IGRPBadgePrimitive variant="outline" className="text-xs">
                      {environments.length}{' '}
                      {environments.length === 1 ? t('variable') : t('variables')}
                    </IGRPBadgePrimitive>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <IGRPBadgePrimitive variant="outline" className="text-xs">
                      {volumes.length} {volumes.length === 1 ? t('volume') : t('volumes')}
                    </IGRPBadgePrimitive>
                  </div>
                  ...
                  <div className="border rounded-md p-3 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{t('volumesFormat')}</p>
                  </div>
                </div>
              </IGRPScrollAreaPrimitive>
            </IGRPTabsContentPrimitive>

            <IGRPTabsContentPrimitive value={t('dependencies')} className="mt-0 space-y-4">
              <div className="space-y-2">
                <IGRPLabelPrimitive>{t('describeServicePlaceholder')}</IGRPLabelPrimitive>
                <p className="text-xs text-muted-foreground">{t('connectedService')}</p>

                <div className="border rounded-md p-3 space-y-2">
                  {services.length > 0 ? (
                    <div className="space-y-2">
                      {services
                        .filter((s) => s.name !== service?.name && !s.labels?.is_project)
                        .map((s, index) => (
                          <div key={index} className="flex items-center space-x-2">
                            <IGRPCheckboxPrimitive
                              id={`depends-${index}`}
                              checked={dependsOn.includes(s.name)}
                              onCheckedChange={() => toggleDependency(s.name)}
                            />
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">{t('selectProjects')}</p>
                        ...
                        .map((p) => (
                          <div key={p.id} className="flex items-center space-x-2">
                            <IGRPCheckboxPrimitive
                              ...
                              onCheckedChange={() => toggleProjectDependency(p.id)}
                            />
                            <div className="flex items-center gap-2">
            ...
            <IGRPTabsContentPrimitive value="network" className="mt-0 space-y-4">
              <div className="space-y-2">
                ...
                <IGRPSelectPrimitive value={networkType} onValueChange={setNetworkType}>
                  <IGRPSelectTriggerPrimitive className="h-8">
                    <IGRPSelectValuePrimitive placeholder={t('selectNetworkType')} />
                  </IGRPSelectTriggerPrimitive>
                  <IGRPSelectContentPrimitive>
                    {networkTypes.map((network) => (
                      <IGRPSelectItemPrimitive key={network.id} value={network.id}>
                        {network.name}
                      </IGRPSelectItemPrimitive>
                    ))}
                  </IGRPSelectContentPrimitive>
                </IGRPSelectPrimitive>
                <p className="text-xs text-muted-foreground">{t('networkDescription')}</p>
              </div>
                ...
                {useCustomNetwork && (
                  <div className="pl-6 space-y-2">
                    ...
                    <IGRPInputPrimitive
                      ...
                      className="h-8"
                    />
                    <p className="text-xs text-muted-foreground">{t('customNetworkName')}</p>
                  </div>
                )}
              </div>
            </IGRPTabsContentPrimitive>

...
import { Database, Globe, Server, HardDrive, Shield, Compass } from 'lucide-react'
...
export function ServiceFilter({ onFilterChange, totalServices }: ServiceFilterProps): JSX.Element {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>
          {totalServices} {totalServices === 1 ? t('service') : t('services')}
          {activeCategory !== 'all' && (
            <span className="ml-1">
              in {serviceCategories.find((c) => c.id === activeCategory)?.name}
            </span>
          )}
        </span>
        {(activeCategory !== 'all' || searchQuery) && (
          <IGRPButtonPrimitive variant="ghost" size="sm" onClick={clearFilters} className="text-xs">
            Clear filters
          </IGRPButtonPrimitive>
        )}
      </div>

      ...
      <div className="text-sm space-y-1">
        ...
        {values.framework === 'springboot' && values.config && (
          <>
            ...
            <p>
              <span className="text-muted-foreground">{t('artifact')}</span>
              {values.config.artifact}
            </p>
            <p>
              <span className="text-muted-foreground">{t('coreVersion')}</span>
              {values.config.coreVersion}
            </p>
            <p>
              <span className="text-muted-foreground">{t('observability')}</span>
              {values.config.enableObservability ? t('enabled') : t('disabled')}
            </p>
          </>
        )}
        {values.framework === 'nextjs' && values.config && (
          <>
            <p>
              <span className="text-muted-foreground">{t('typescript')}</span>
              {values.Config.typescript ? 'Yes' : 'No'}
            </p>
            <p>
              <span className="text-muted-foreground">{t('tailwindCss')}</span>
              {values.Config.tailwind ? 'Yes' : 'No'}
            </p>
            <p>
              <span className="text-muted-foreground">{t('srcDirectory')}</span>
              {values.Config.srcDirectory ? 'Yes' : 'No'}
            </p>
            <p>
              <span className="text-muted-foreground">{t('packageManager')}</span>
        ...
        {values.framework === 'dotnet' && values.Config && (
          <>
            ...
            <p>
              <span className="text-muted-foreground">{t('solutionName')}</span>
            ...
            <p>
              <span className="text-muted-foreground">{t('https')}</span>
              {values.Config.https ? 'Yes' : 'No'}
            </p>
            <p>
              <span className="text-muted-foreground">{t('dockerSupport')}</span>

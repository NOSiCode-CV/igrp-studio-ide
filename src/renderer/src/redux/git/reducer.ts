...
export const gitSlice = createSlice({
    ...
    reducers: {
        ...
        setGitLabProviders: (
            state,
            action: PayloadAction<GitLabProvider[]>
        ) => {
        ...
        updateGitLabProvider: (
            ...
            action: PayloadAction<{
                id: string;
                updates: Partial<GitLabProvider>;
            }>
        ) => {
            const { id, updates } = action.payload;
            const providerIndex = state.gitLabProviders.findIndex(
                (p) => p.id === id
            );
            if (providerIndex !== -1) {
        ...
        removeGitLabProvider: (state, action: PayloadAction<string>) => {
            state.gitLabProviders = state.gitLabProviders.filter(
        ...
        setActiveProvider: (state, action: PayloadAction<string | null>) => {
            ...
            if (action.payload && action.payload !== 'github') {
                const provider = state.gitLabProviders.find(
                    (p) => p.id === action.payload
                );
                if (provider) {
        ...
        setProviderUser: (
            state,
            action: PayloadAction<{ providerId: string; user: any }>
        ) => {

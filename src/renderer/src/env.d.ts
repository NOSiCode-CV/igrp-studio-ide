...
interface SpecSettingsBridge {
  ...
  setSecret: (
    ...
    value: string,
  ) => Promise<SpecSecretsStatus>;
  testSecret: (
    provider: SpecSecretProvider,
  ) => Promise<{ ok: boolean; error?: string }>;
  getPreferences: () => Promise<SpecPreferencesPatch>;
  setPreferences: (
    patch: SpecPreferencesPatch,
  ) => Promise<SpecPreferencesPatch>;
}
...
interface SpecPrototypeBridge {
  ...
  onChunk: (
    callback: (payload: {
      requestId: string;
      chunk: SpecPrototypeChunk;
    }) => void,
  ) => () => void;
  onDevLog: (
    callback: (payload: {
      basePath: string;
      entry: SpecPrototypeDevLog;
    }) => void,
  ) => () => void;
  onDevStatus: (
    callback: (payload: {
      basePath: string;
      status: SpecPrototypeDevStatus;
    }) => void,
  ) => () => void;
  onTreeChanged: (
...
interface Window {
  ...
  graphql: {
    createGraphQLOperation: (
      ...
      import("./generators/api/pages/graphql/types").GraphQLPersistedOperation
    >;
    updateGraphQLOperation: (
      ...
      operationId: string,
      updates: Partial<
        import("./generators/api/pages/graphql/types").GraphQLOperationPayload
      >,
    ) => Promise<
      import("./generators/api/pages/graphql/types").GraphQLPersistedOperation
    >;

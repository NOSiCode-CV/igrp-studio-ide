...
class BPMNService {
    ...
    private async makeRequest<T>(
        ...
        if (!activeConfig) {
            throw new Error(
                'BPMN API not configured. Please set up the API configuration first.'
            );
        }
    ...
    async getActiveConfig(): Promise<BPMNConfig | null> {
        ...
        if (configs.activeConfigId) {
            const activeConfig = configs.configs.find(
                (c) => c.id === configs.activeConfigId
            );
            if (activeConfig) {
    ...
    async testConnection(
        config: Omit<BPMNConfig, 'id' | 'createdAt' | 'status'>
        ...
        try {
            ...
            const result: HandlerResponse = await window.api.fetchData(
                `${config.apiUrl}${config.basePath}/projects`,
                {
                    headers,
                }
            );
    ...
    async getProjects(): Promise<BPMNProject[]> {
        const response =
            await this.makeRequest<PaginatedResponse<BPMNProject>>('/projects');
        return response ? response.content : [];
    }

    // New method to get process definitions for a specific project
    async getProcessDefinitionsByProject(
        projectId: string
    ): Promise<BPMNProjectProcessDefinition[]> {
        const response = await this.makeRequest<BPMNProject>(
            `/projects/${projectId}/deployed-process`
        );
    ...
    async getProcessDefinitionXML(
        ...
        for (const project of projects) {
            const processDef = project.processDefinitions?.find(
                (pd) => pd.processDefinitionId === processDefinitionId
            );
            if (processDef) {
        ...
        if (!projectId) {
            throw new Error(
                `Process definition ${processDefinitionId} not found in any project`
            );
        }
    ...
    async getProcessDefinitionDetails(
        processDefinitionId: string
    ): Promise<any> {
        // Get process definition details directly
        return await this.makeRequest<any>(
            `/projects/process-definitions/${processDefinitionId}`
        );
    }
    ...
    async getProcessArtifacts(processDefinitionId: string): Promise<any[]> {
        try {
            const processDetails =
                await this.getProcessDefinitionDetails(processDefinitionId);
    ...
    async getArtifactContent(artifactId: string): Promise<{ content: string }> {
        try {
            ...
            const artifact = await this.makeRequest<{ content: string }>(
                `/projects/artifacts/${artifactId}/content`
            );
            return artifact;
        } catch (error) {
            console.error(
                `Error fetching content for artifact ${artifactId}:`,
                error
            );

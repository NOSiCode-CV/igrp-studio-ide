...
const repo = {
  workspace: {
    ...
    createProject: async (
      ...
      try {
        return await ipcRenderer.invoke(EVENTS.REPOSITORY.PROJECT.CREATE, workspaceId, project)
      } catch (error) {
        return handleError(error)
      }
    },
    updateProject: async (projectId: string, updates: Partial<ProjectData>) => {
      try {
        return await ipcRenderer.invoke(EVENTS.REPOSITORY.PROJECT.UPDATE, projectId, updates)
      } catch (error) {
        return handleError(error)
      }
    },
    ...
    getProject: (projectId: string) => ipcRenderer.invoke(EVENTS.REPOSITORY.PROJECT.GET, projectId),

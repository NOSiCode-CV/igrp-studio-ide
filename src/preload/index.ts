...
const api = {
    ...
    openDirectory: (buttonLabel?: string) =>
        ipcRenderer.invoke('open-directory', buttonLabel),

    fetchFiles: (basePath: string) =>
        ipcRenderer.invoke('igrp-studio:fetch-files', basePath),
    ...
    readDirectory: (basePath: string) =>
        ipcRenderer.invoke('read-directory', basePath),
    ...
    getVersions: (endpoint: string) =>
        ipcRenderer.invoke('get-versions', endpoint),
    ...
    runDoctorChecks: (): Promise<ToolCheck[]> =>
        ipcRenderer.invoke('run-doctor-checks'),
    saveDoctorReport: (results) =>
        ipcRenderer.invoke('save-doctor-report', results),
...
const appLogic = {
    ...
    addEnvironment: (environment) =>
        ipcRenderer.invoke(EVENTS.APPLOGIC.CREATE, environment),
    updateEnvironment: (id, updates) =>
        ipcRenderer.invoke(EVENTS.APPLOGIC.UPDATE, id, updates),
    ...
    // History
    addConnectionTest: (test) =>
        ipcRenderer.invoke('app-logic:add-connection-test', test),
    ...
    importData: (jsonData) =>
        ipcRenderer.invoke('app-logic:import-data', jsonData),

    // Test
    testEnvironment: (environment) =>
        ipcRenderer.invoke(EVENTS.APPLOGIC.TEST, environment),
    ...
    onEnvironmentsChanged: (callback) => {
        ...
        return () =>
            ipcRenderer.removeListener(EVENTS.APPLOGIC.CHANGE, subscription);
    },

    removeAllListeners: () =>
        ipcRenderer.removeAllListeners(EVENTS.APPLOGIC.CHANGE),
};
...
const igrpStudioSettings = {
    ...
    addBPMNConfig: (config: BPMNConfig) =>
        ipcRenderer.invoke(EVENTS.BPMN.ADD_CONFIG, config),
    updateBPMNConfig: (config: BPMNConfig) =>
        ipcRenderer.invoke(EVENTS.BPMN.UPDATE_CONFIG, config),
    deleteBPMNConfig: (configId: string) =>
        ipcRenderer.invoke(EVENTS.BPMN.DELETE_CONFIG, configId),
    setActiveBPMNConfig: (configId: string) =>
        ipcRenderer.invoke(EVENTS.BPMN.SET_ACTIVE_CONFIG, configId),
    deleteAllBPMNConfigs: () =>
        ipcRenderer.invoke(EVENTS.BPMN.DELETE_ALL_CONFIGS),

    // Language methods
    getLanguage: () => ipcRenderer.invoke(EVENTS.LANGUAGE.GET_LANGUAGE),
    setLanguage: (lang: string) =>
        ipcRenderer.invoke(EVENTS.LANGUAGE.SET_LANGUAGE, lang),
};
    ...
    try {
        contextBridge.exposeInMainWorld('electron', {
            ...
            reportError: (error: Error) =>
                ipcRenderer.send('report-error', error),
        });
